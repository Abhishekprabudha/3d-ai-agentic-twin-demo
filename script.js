...
function writeStaticSummary(data, label) {
  if (data?.commentary?.static) log(`${label}: ${data.commentary.static}`);
  else log(`${label}: ${(data?.warehouses||[]).length||3} warehouses, ${(data?.trucks||[]).length||0} trucks.`);
}
...
function buildLabels() {
  ...
  function place(text, basePos, pushOut=7.0, angle=0) {
    ...
    spr.position.set(p.x, offsetY, p.z);
    LABELS.add(spr);
  }
...
function defaultPathIDs(origin, destination) {
  ...
  if (ADJ[origin]?.includes(destination)) return [origin, destination];
...
function createTruckMesh(delayed) {
  ...
  const body = new THREE.Mesh(bodyGeo, bodyMat); body.position.set(0,0.9,0); group.add(body);

  const cabGeo = new THREE.BoxGeometry(1.0,1.1,1.2);
  const cabMat = new THREE.MeshLambertMaterial({ color: cabColor });
  const cab = new THREE.Mesh(cabGeo, cabMat); cab.position.set(-1.8,0.85,0); group.add(cab);
  ...
  const wheels=[];
  function addWheel(x,z){ const w=new THREE.Mesh(wheelGeo,wheelMat); w.rotation.z=Math.PI/2; w.position.set(x,0.4,z); group.add(w); wheels.push(w); }
...
function segmentProgress(t) {
  const a = t.path[t.segIdx], b = t.path[t.segIdx + t.direction];
  if (!a || !b) return 0;
  const segLen = a.distanceTo(b);
  return t.segT * segLen;
}
function nearJunction(pos) {
  for (const key in WH_POS) if (pos.distanceTo(WH_POS[key]) < JUNCTION_RADIUS) return true;
  return false;
}
...
async function loadScenario(file, labelFromCaller) {
  try {
    ...
    if (Array.isArray(data.reroutes)) {
      for (const r of data.reroutes) if (Array.isArray(r.path) && r.truckId) rerouteMap.set(r.truckId, r.path.slice());
    }
    ...
    writeStaticSummary(data, labelFromCaller || (isAfter ? "After correction" : "Normal operations"));
  ...
  for (const t of movingTrucks) {
    ...
    if (candidates.length) {
      ...
      for (const o of candidates) {
        ...
        if (oProg > myProg) minGap = Math.min(minGap, oProg - myProg);
      }
